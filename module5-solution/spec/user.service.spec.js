describe('user', function () {
  let existingItemShortName;
  let notExistingItemShortName;

  let expectedMenuItem;
...
VG: {
  ...
  beforeEach(function () {
    ...
    existingItemShortName = "A1";
    notExistingItemShortName = "A99";
    ...
    inject(function ($injector) {
      menucategories = $injector.get('UserService');
      $httpBackend = $injector.get('$httpBackend');
    });
  });

  it('should return an object', function() {  // the tested function should find the user preferred menu item and then return it
    $httpBackend.whenGET("https://coursera-jhu-default-rtdb.firebaseio.com/menu_items.json").respond(jsonResponse);
    menucategories.getUserPreferredMenuItem(existingItemShortName).then(function(response) {
      expect(response).toEqual({menuItem: expectedMenuItem, menuItemCategory: expectedCategory});
    });

    $httpBackend.flush();
  });

  it('should return false', function() {  // the tested function shouldn't find the user preferred menu item and then return false
    $httpBackend.whenGET("https://coursera-jhu-default-rtdb.firebaseio.com/menu_items.json").respond(jsonResponse);
    menucategories.getUserPreferredMenuItem(notExistingItemShortName).then(function(response) {
